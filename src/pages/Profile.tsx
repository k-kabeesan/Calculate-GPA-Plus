...
export function ProfilePage({ id }: { id: string }) {
  ...
  return <div className="container"><PageHead eyebrow="PUBLIC ACADEMIC PROFILE" title={profile.name}>{profile.university} · {profile.faculty}</PageHead>
    ...
    </div><aside><section className="panel result-panel stack"><div><h2>Your result</h2><p>Your grade choices stay in this browser.</p></div><GpaSummary result={result} label="CGPA" /><label>Student name for PDF<input value={studentName} onChange={event => setStudentName(event.target.value)} placeholder="Optional" /></label><button className="button button-soft full" onClick={async () => (await import('../services/pdf')).buildReport(personal!, studentName)}>Download PDF</button></section></aside></div>
